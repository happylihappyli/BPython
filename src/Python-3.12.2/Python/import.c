...
init_importlib_external(PyInterpreterState *interp)
{
    ...
    PyObject *raw_magic = PyObject_GetAttrString(external, "_RAW_MAGIC_NUMBER");
    if (raw_magic == NULL) {
        PyErr_Clear();
        PyObject *magic = PyObject_GetAttrString(external, "MAGIC_NUMBER");
        if (magic != NULL && PyBytes_Check(magic)) {
            unsigned long num = 0;
            unsigned char *buf = (unsigned char *)PyBytes_AS_STRING(magic);
            Py_ssize_t len = PyBytes_GET_SIZE(magic);
            for (Py_ssize_t i = 0; i < len && i < (Py_ssize_t)sizeof(unsigned long); i++) {
                num |= ((unsigned long)buf[i]) << (8 * i);
            }
            PyObject *val = PyLong_FromUnsignedLong(num);
            if (val != NULL) {
                (void)PyObject_SetAttrString(external, "_RAW_MAGIC_NUMBER", val);
                Py_DECREF(val);
            }
        }
        Py_XDECREF(magic);
    }
    Py_XDECREF(raw_magic);
    Py_DECREF(external);
    return 0;
}

...
#define MAKE_TOKEN(token_type) (tok->previous_token_kind = token_type, token_setup(tok, token, token_type, p_start, p_end))
...
tok_new(void)
{
    ...
    tok->col_offset = -1;
    tok->level = 0;
    tok->previous_token_was_block_starter = 0;
    tok->altindstack[0] = 0;
    tok->previous_token_kind = 0;
                   ...
                   va_list vargs)
{
    ...
    // DEBUG PRINT
    {
        PyObject* bytes = PyUnicode_AsUTF8String(errmsg);
        if (bytes) {
            printf("DEBUG: syntaxerror message: %s\n", PyBytes_AsString(bytes));
            Py_DECREF(bytes);
        }
        if (tok->filename) {
            PyObject* fnbytes = PyUnicode_AsUTF8String(tok->filename);
            if (fnbytes) {
                printf("DEBUG: file: %s line: %d\n", PyBytes_AsString(fnbytes), tok->lineno);
                Py_DECREF(fnbytes);
            }
        }
        if (tok->line_start && tok->cur >= tok->line_start) {
            PyObject* line = PyUnicode_DecodeUTF8(tok->line_start, tok->cur - tok->line_start, "replace");
            if (line) {
                PyObject* linebytes = PyUnicode_AsUTF8String(line);
                if (linebytes) {
                    printf("DEBUG: line text: %s\n", PyBytes_AsString(linebytes));
                    Py_DECREF(linebytes);
                }
                Py_DECREF(line);
            }
...
error:
    Py_XDECREF(errmsg);
    printf("DEBUG: Setting E_ERROR in _syntaxerror_range\n");
    tok->done = E_ERROR;
    return ERRORTOKEN;
}
...
error:
    Py_XDECREF(errmsg);
    printf("DEBUG: Setting E_ERROR in parser_warn\n");
    tok->done = E_ERROR;
    return -1;
}
...
verify_identifier(struct tok_state *tok)
{
    ...
    if (invalid < 0) {
        Py_DECREF(s);
        printf("DEBUG: Setting E_ERROR in verify_utf8 (2)\n");
        tok->done = E_ERROR;
        return 0;
    }
    ...
    if (invalid < PyUnicode_GET_LENGTH(s)) {
        ...
        if (invalid + 1 < PyUnicode_GET_LENGTH(s)) {
            ...
            if (s == NULL) {
                printf("DEBUG: Setting E_ERROR in verify_utf8 (3)\n");
                tok->done = E_ERROR;
                return 0;
            }
...
token_setup(struct tok_state *tok, struct token *token, int type, const char *start, const char *end)
{
    ...
    // BPython: Update previous_token_was_block_starter
    int is_block_starter = 0;
    if (type == RPAR || type == RBRACE || type == RSQB || type == STRING || type == NUMBER) {
        is_block_starter = 1;
    } else if (type == NAME && start != NULL && end != NULL) {
        size_t len = end - start;
        // Check for specific keywords that start blocks
        if ((len == 3 && strncmp(start, "try", 3) == 0) ||
            (len == 4 && strncmp(start, "else", 4) == 0) ||
            (len == 7 && strncmp(start, "finally", 7) == 0) ||
            (len == 6 && strncmp(start, "except", 6) == 0) ||
            (len == 2 && strncmp(start, "do", 2) == 0)) {
            is_block_starter = 1;
        } else {
            // Check for keywords that expect expressions/statements (not blocks)
            if ((len == 6 && strncmp(start, "return", 6) == 0) ||
                (len == 5 && strncmp(start, "yield", 5) == 0) ||
                (len == 5 && strncmp(start, "raise", 5) == 0) ||
                (len == 6 && strncmp(start, "import", 6) == 0) ||
                (len == 6 && strncmp(start, "global", 6) == 0) ||
                (len == 8 && strncmp(start, "nonlocal", 8) == 0) ||
                (len == 6 && strncmp(start, "assert", 6) == 0) ||
                (len == 3 && strncmp(start, "del", 3) == 0) ||
                (len == 4 && strncmp(start, "pass", 4) == 0) ||
                (len == 5 && strncmp(start, "break", 5) == 0) ||
                (len == 8 && strncmp(start, "continue", 8) == 0)) {
                is_block_starter = 0;
            } else {
                // All other identifiers (variables, func names, match subject) are potential block starters
                is_block_starter = 1;
            }
        }
    }
    tok->previous_token_was_block_starter = is_block_starter;
...
tok_get_normal_mode(struct tok_state *tok, tokenizer_mode* current_tok, struct token *token)
{
    ...
    if (tok->filename) {
        PyObject *filename_obj = tok->filename;
        if (PyUnicode_Check(filename_obj)) {
            const char *filename_utf8 = PyUnicode_AsUTF8(filename_obj);
            if (filename_utf8) {
                size_t len = strlen(filename_utf8);
                if (len > 4 && strcmp(filename_utf8 + len - 4, ".bpy") == 0) {
                    is_bpy = 1;
                }
                debug_filename = filename_utf8;
            }
        }
    }
  ...
  nextline:
    ...
    if (tok->atbol) {
        ...
        if (!blankline && debug_filename && (strstr(debug_filename, "_arraypad_impl.py") || strstr(debug_filename, "test_match.py"))) {
             int skipped = !(tok->level == 0 || (tok->level > 0 && tok->parenstack[tok->level - 1] == BLOCK_BRACE_MARKER));
             if (skipped) {
                 printf("DEBUG: SKIPPED indentation. line=%d, level=%d, stack top=%d\n", tok->lineno, tok->level, tok->level > 0 ? tok->parenstack[tok->level-1] : -1);
             } else {
                 printf("DEBUG: CHECK indentation. line=%d, col=%d, current_indent=%d\n", tok->lineno, col, tok->indstack[tok->indent]);
             }
         }
        if (!blankline && (tok->level == 0 || (tok->level > 0 && tok->parenstack[tok->level - 1] == BLOCK_BRACE_MARKER))) {
    ...
    if (c == '\n') {
        ...
        if (tok->level > 0) {
            ...
            for (i = 0; i < tok->level; i++) {
                if (tok->parenstack[i] != BLOCK_BRACE_MARKER) {
                    suppress_newline = 1;
                    break;
                }
            }
        }
        if (blankline || suppress_newline) {
            if (tok->tok_extra_tokens) {
  ...
  letter_quote:
    ...
    if (is_punctuation && INSIDE_FSTRING(tok) && INSIDE_FSTRING_EXPR(current_tok)) {
        ...
        if (c == ':' && cursor == current_tok->curly_bracket_expr_start_depth &&
            (tok->level == 0 || (tok->parenstack[tok->level - 1] != '[' && tok->parenstack[tok->level - 1] != '('))) {
    ...
    case '{':
        ...
        if (is_bpy && c == '{' && tok->previous_token_was_block_starter) {
            tok->parenstack[tok->level] = BLOCK_BRACE_MARKER;
        } else {
    ...
    case '}':
        ...
        if (tok->level > 0) {
            ...
            if (!tok->tok_extra_tokens && !((opening == '(' && c == ')') ||
                                            (opening == '[' && c == ']') ||
                                            (opening == '{' && c == '}') ||
                                            (opening == BLOCK_BRACE_MARKER && c == '}'))) {
                /* If the opening bracket belongs to an f-string's expression
                part (e.g. f"{)}") and the closing bracket is an arbitrary
                nested expression, then instead of matching a different
                syntactical construct with it; we'll throw an unmatched
                parentheses error. */
                if (INSIDE_FSTRING(tok) && (opening == '{' || opening == BLOCK_BRACE_MARKER)) {
...
tok_get(struct tok_state *tok, struct token *token)
{
    ...
    if (current_tok->kind == TOK_REGULAR_MODE) {
        type = tok_get_normal_mode(tok, current_tok, token);
    } else {
        type = tok_get_fstring_mode(tok, current_tok, token);
    }
    
    if (tok->filename) {
        PyObject *filename_obj = tok->filename;
        if (PyUnicode_Check(filename_obj)) {
            const char *fn = PyUnicode_AsUTF8(filename_obj);
             if (fn && (strstr(fn, "test_match.py") || strstr(fn, "test_if.py") || strstr(fn, "test_if_indent.py"))) {
                 printf("TOKEN: %d, line=%d, col=%d, start=%d, end=%d\n", type, token->lineno, token->col_offset, (int)(token->start - tok->line_start), (int)(token->end - tok->line_start));
             }
        }
    }
    return type;

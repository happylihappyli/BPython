...
def _enable_bpy_import():
    """Enable importing of modules with .bpy extension and refresh import hooks"""
    import importlib._bootstrap_external as be
    import sys
    print("DEBUG: site.py be id:", id(be))
    
    if '.bpy' not in be.SOURCE_SUFFIXES:
        be.SOURCE_SUFFIXES.append('.bpy')
    
    # Force refresh of path hooks by re-adding the FileFinder hook
    # This ensures the hook uses the updated SOURCE_SUFFIXES
    try:
        if hasattr(be, '_get_supported_file_loaders'):
            loaders = be._get_supported_file_loaders()
            hook = be.FileFinder.path_hook(*loaders)
            sys.path_hooks.insert(0, hook)
    except Exception as e:
        print(f"Error updating path hooks: {e}")
